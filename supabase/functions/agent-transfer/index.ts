...
<Response>
  ...
  <Say>The agent is not available at the moment. Please try again later or leave a message.</Say>
    ...
    // CRITICAL: Use Twilio REST API to UPDATE the active call with new TwiML
    // This disconnects the WebSocket and executes the Dial command
    try {
      ...
      if (!updateResponse.ok) {
        ...
        throw new Error(`Twilio API failed: ${updateResponse.status} - ${errorText}`);
      }

      const updateResult = await updateResponse.json();
      console.log('Call updated successfully:', updateResult);
      ...
      console.error('Twilio call update failed:', twilioError);
